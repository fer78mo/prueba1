...
# Bloqueo por ley (si el enunciado cita una ley y la mejor evidencia es de otra → descartar)
...
def _detect_reference(enunciado: str) -> Optional[dict]:
    """Detecta referencia directa a artículo/disposición/anexo y pista de ley en el enunciado."""
    t = enunciado.lower()
    pieza_tipo = "articulo"
    ...
    # Artículo N [bis/ter/...]
    m = re.search(
        r"\bart(?:[íi]culo|\.)\s+(\d+)\s*(bis|ter|quater|quinquies|sexies|septies|octies|nonies|decies)?\b",
        t,
    )
    if m:
        num = int(m.group(1))
...
# ---------- solape “inteligente” para opciones cortas ----------
...
def _loose_option_overlap(quote: str, option_txt: str, min_ratio: float = 0.08) -> bool:
    """
    Si la opción es “normal” (>=10 chars y >2 tokens), usa el chequeo estándar.
    Si la opción es corta (≤2 tokens o <10 chars), validamos por “raíz” (primeros 8 chars).
    """
    opt_norm = _norm_str(option_txt or "")
    toks = opt_norm.split()
    if len(opt_norm) >= 10 and len(toks) > 2:
        return option_overlap_support(quote or "", option_txt or "", min_ratio=min_ratio)
    roots = [t[:8] for t in toks if t]
    qn = _norm_str(quote or "")
    return any(r and r in qn for r in roots)
...
def _pick_and_quote(
    ...
    if mode != "incorrecta":
        ...
        for idx, score in order[:6]:
            p = payloads[idx]
            # Ley-guard: si no corresponde, salta
            ...
            ok_overlap = _loose_option_overlap(quote, option_txt, min_ratio=0.08)
        ...
        # sin cita válida → fallback controlado por ley-guard
        fb_idx = _first_fallback_idx(order, payloads, expected_ley_id)
        return "", payloads[fb_idx], float(order[0][1]), {}

    # Modo "incorrecta": base normativa relevante al enunciado
    for idx, score in order[:8]:
        p = payloads[idx]
        if not _guard_match(expected_ley_id, p.get("ley_id")):
            continue
        t = texts[idx]
        minlen = _min_quote_len(t, mode)  # típicamente 60 en incorrecta para párrafos cortos
...
def solve_question(enunciado: str, opcion_correcta_texto: str, mode: str = "correcta") -> Tuple[str, dict]:
    ...
    # 1) Directo por referencia (TXT)
    if ref.get("ley_id"):
        try:
            hits = search_txt_by_ref(
                ref["ley_id"],
                ref.get("pieza_tipo", "articulo"),
                ref.get("num"),
                ref.get("sufijo"),
            )
        ...
        if hits:
            quote, payload, s, span = _pick_and_quote(
                enunciado, opcion_correcta_texto, hits, alias, expected_ley_id, mode
            )
    ...
    # 2) Shortlist de leyes (TXT, fusión BM25 ya en retriever)
